...
const AuctionGrid = ({ auctions, currentUser, onAdminRemove }) => {
  ...
  return (
    <section className="py-12 bg-gray-50 dark:bg-slate-900 transition-colors duration-200">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 space-y-4 lg:space-y-0">
          ...
          <div className="flex items-center space-x-4">
            ...
            <select
              ...
              className="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 text-gray-900 dark:text-gray-100 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
            >
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg dark:shadow-slate-900/50 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 overflow-hidden group cursor-pointer flex flex-col h-full w-full">
        ...
        <div className="p-6 flex flex-col flex-grow">
          <div className="flex justify-between items-start mb-2">
            <h3 className="font-bold text-lg text-gray-800 dark:text-gray-100 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors line-clamp-2 flex-1">{auction.title}</h3>
          ...
          <p className="text-gray-600 dark:text-gray-400 text-sm mb-4 line-clamp-2 flex-shrink-0">{auction.description}</p>

          <div className="space-y-3 flex-grow">
            <div className="flex justify-between items-center">
              <span className="text-gray-500 dark:text-gray-400 text-sm">Current Bid</span>
              <span className="text-2xl font-bold text-gray-800 dark:text-gray-100">${auction.currentPrice.toLocaleString()}</span>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-700 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                <div className="w-8 h-8 bg-gradient-to-r from-blue-400 to-purple-500 rounded-full flex items-center justify-center text-white text-xs font-bold">{auction.seller.charAt(0)}</div>
                <span className="text-sm text-gray-600 dark:text-gray-300">{auction.seller}</span>
              </div>
              <button className="text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 font-medium text-sm">Place Bid</button>
            </div>
          </div>
        </div>
      </div>
